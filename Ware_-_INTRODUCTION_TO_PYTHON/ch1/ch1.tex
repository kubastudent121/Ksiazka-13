...
\chapter*{Ackonowledgements}
		...
		their permission, I've adapted their material to our use here at BYU Provo. In
		...
		into development this material.I've put my name as author of this edition to
	...
	\chapter{Numpy, Scipy, and Plotting}
		\section{Get Anaconda Installed}
		...
		\begin{itemize}
		\item The code editor on the left is where you edit your Python code files (usually with a .py extension).
		
  		\item The top right pane has three default tabs a variable explorer where you can view current values stored in memory, a file explorer where you can browse your files, and a help tab where you can ask questions
  		
 		\item The lower right pane is a console window where your output will be displayed and you can issue Python commands directly to be evaluated
  		\ldots 
	...
	\marginpar{
\includegraphics[width=\marginparwidth]{latex12}
\captionof{figure}[] {The Spyder IDE window.}\label {Figure 1.1}
}
	...
	\section{The Python Console}
		Spyders console window is a powerful environment called Interactive Python 
	...
	\begin{lstlisting}[language=Python,numbers=none,showstringspaces=false]
		Out[1]+2
	\end{lstlisting}
	Notice that your new result is stored in Out[2]. IPython behaves a lot like a Matlab's command window, with a series of inputs and variable values accumulating
	in the workspace. When you run your Python programs, your variables are placed
	in the console workspace and can be accessed from the command line afterward. 
	...
	\end{lstlisting}
		...
		of 20. Python did not print anything since we didn't include a print command. To 
		convince yourself that the variable x was defined, click on the ''Variable explorer'' 
	...
	\end{lstlisting}
		...
		{Table 1.1: A sampling of built$-$in functions commonly used with integers and floats.}\label{Table 1.1}
...
\end{tabular}
	...
	your cursor on the function name in your code and press Ctrl-I (think '' I is for
	...
	\section{Boolean variables}
		...
		variable is created and assigned similar to the other variables you've studied so
		far
	\begin{lstlisting}[language=Python,numbers=none,showstringspaces=false]
		q = True
	\end{lstlisting}
	...
	\section{Formatting Printed Values}
	\begin{lstlisting}[language=Python,numbers=none,showstringspaces=false]
		a = 22
		b = 3.5
...
a and b.
\end{tabular}
 { Table 1.2: A sampling of ''house$-$keeping'' functions for strings.}\label{Table 1.2}
 ...
 \begin{tabular}{  m{1cm} m{2cm} }
	...
	the decimal
\end{tabular}
{ Table 1.3: Formatting strings available when printing.}\label{Table 1.3}
}

\section{Functions and Libraries (Numpy)}
...
\end{lstlisting}
	...
	scientific calculation can be found in a library called NumPy (say ''num pie'' ,
...
\end{lstlisting}
		This syntax tells Python ''{}I' m going to call the numpy library np.{} We recommend %%\“{}I\’{}
...
\end{lstlisting}
	...
	you see functions with the ''{}np.''{} prefix.\\
	...
	called SciPy (say ''sigh pie'' , not '' skippy''). These two libraries are designed to work
...
\end{lstlisting}
	If numpy doesn't have a mathematical function you need, then scipy.special
	probably has it. \hyperref[Table 1.5]{Table 1.5} provides a small sampling of its functions. See \textcolor{blue}{SciPy's
...
\marginpar{
	...
	log10(x)\\
	log2(x)\\
{ Table 1.4: A very small sampling of
...
\end{lstlisting}
	...
	\end{lstlisting}
		...
		When you use the linspace function, you aren't specifying a step size. You can
...
\end{tabular}
{ Table 1.5:A very small sampling
...
\subsection*{Functions of Arrays}
...
\end{lstlisting}
	The math library is much less capable than numpy, so we don't recommend using
...
\subsection*{Accessing Elements of Numpy Arrays}
...
\end{tabular}
{ Table 1.6: A sampling of arraybuilding functions in numpy. The
...
\end{tabular}
	...
	\textcolor{green}{'ro'} inside of the plot command
\begin{lstlisting}[language=Python,numbers=none,showstringspaces=false]
	plt.plot(x,y,'ro')
\end{lstlisting}
	The \textcolor{green}{'r'} means make the data points red and the \textcolor{green}{'o'} means plot circle markers.
...
\begin{enumerate}
	...
	of execution of your program depends on what you've manually entered in the console. When learning to program, we recommend that you have
	Spyder clear all variables before executing a program. To do this go to the
	Tools menu in Spyder and select Preferences $\rightarrow$ Run and check the '' Remove
	all variables before execution'' option.}
\end{enumerate}
\end{document}
