...
\begin{document}
\chapter{Lists, Loops, Logic, and 2D Arrays}
\section{Lists}
	You'll encounter lists quite a bit in Python. In fact we already used some in the
...
the elements of a\\
\end{tabular}
{Table 2.1 A sampling of ''house$-$keeping'' functions for lists.
}\label{Table 2.1}
}
	When looking at the numerical lists above, you might be tempted to think of
lists as mathematical matrices.Don't. Lists are not really designed for mathematics. To convince yourself of this, add the statement
	\begin{lstlisting}[language=Python,numbers=none,showstringspaces=false]
	a[1][2] = 'george'
	\end{lstlisting}
	after the list of lists above and then examine a. You can't calculate on elements
that aren't numbers! As another example, execute this code
	\begin{lstlisting}[language=Python,numbers=none,showstringspaces=false]
	import numpy as np
	...
	Python's range function represents a sequence of integer values that behaves a
	...
	c = range(10) # starts at 0 (default), ends at 9, 
	steps of 1 (default)
	\end{lstlisting}
	\section{For Loops and Shorthand Assignments}
...
use it with a for loop to index a separate array, like this
	...
	for y in x:
		print(y)
	\end{lstlisting}
	To practice \textcolor{violet}{for} loops, let's build a loop to calculate the sum
...
\end{lstlisting}
	...
	should be equal but actually aren't equal in the computer. The take home here
...
{Table 2.3:  Python's logic elements.
	...
	when you don't know beforehand exactly how many iterations of the loop will be
...
\subsection*{The continue statement}
	...
	while term > 1e-10: # Loop while term is bigger than 1e-10
		n += 1 # Add 1 to n so that it will count: 2,3,4,5
		if n % 3 != 0:
			continue # Skip the rest of the code, start the 
			next iteration
		term = 1./n**2 # Calculate the next term to add
		s += term # Add 1/n^2 to the running total
	\end{lstlisting}
...
\section{2D NumPy Arrays $-$ Matrices}
...
\subsection*{Creating Larger Matrices}
...
\end{lstlisting}
	...
	array's shape property, like this
\begin{lstlisting}[language=Python,numbers=none,showstringspaces=false]
	s = A.shape
\end{lstlisting}
...
\subsection*{Math with Matrices}
	...
	standard linear algebra functions with SciPy's linear algebra module. The matrix
...
\section{Solving a Set of Linear Equations}
	To illustrate the use of matrix computations, let's solve a set of linear equations.
