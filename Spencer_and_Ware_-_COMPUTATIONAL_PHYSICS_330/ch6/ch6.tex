...
the behavior of this system\textsuperscript{1}. Before we dive into the computational details, let's
...
\item  Solve \hyperref[6.1]{Eq. 6.1} numerically using Matlab's ODE solver with initial values $x_0 ~= ~1~ and~ v_0 ~=~ 0,
~ v_0 ~=~ 1.4,~ and ~v_0~ =~ −1$ and run them from $t = 0 ~to~ ~t = 1$. Overlay the three trajectory plots on your flow plot. Identify
...
\end{equation}
Equation (\hyperref[6.5]{6.5}) looks impressive, but if it's supposed to be an oscillator that damps,
where are the sines and cosines? The problem is that we haven't specified how
big $\omega _ 0$ and $\gamma$ are yet. Let's think physically for a minute.\\
...
sines and cosines? Recall Euler's formula
$$
...
\subsection*{P6.2}\label{P6.2}
\begin{enumerate}[label=\alph*]
\item Use Matlab to make a phase$-$space plot for a damped harmonic oscillator with $\omega _ 0 = 2 ~and~ \gamma = 0.5.$
\item Solve \hyperref[6.4]{Eq. 6.4} numerically using Matlab's ODE solver with initial values $x_0 = 1 ~and~ v_0 = 0, v_0 = 1.4,~ and~ v_0 = −1$, and run from $t = 0$ to
...
It is possible to solve \hyperref[6.9]{Eq. 6.9} symbolically, but let's study its behavior numerically for practice.
\subsection*{P6.3}\label{P6.3}
...
phase difference between the driving force and the oscillator's response. By
