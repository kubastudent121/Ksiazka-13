...
\chapter{Two Gravitating Bodies}
	...
	Let's continue our study of differential equations by considering two masses
	interacting through Newton's law of gravity\textsuperscript{1} The Newton's second$-$law equations
	...
	\begin{enumerate}[label=\alph*]
		...
		it doesn't show what the dynamics of this system are like. For that, it would be
		...
		The problem with making movies directly from the data returned by Matlab's
		ODE solvers is that the data that Matlab's ODE solver returns is not equally spaced
...
\marginpar{
\includegraphics[scale = 0.6]{ch8zd}
\captionof{figure}[] { Two masses interacting
...
\subsection*{\small{P8.3}}\label{P8.3}
 Now let's go back to your code from \hyperref[P8.1]{P8.1}. After obtaining the solution
...
\end{lstlisting}
	...
	points makes the plots be ''jerky.'' A loop that will do this kind of animation
	is shown below:
\begin{lstlisting}[language=Matlab][numbers=none]
	for n=5:4:N
...
\section{Linear Algebra}
	...
	\begin{enumerate}[label=\alph*]
		\item  Use Matlab's dot command to find the angle between the vectors
		$A = [1, 2, 3]$ and $B = [−3, 2, 1]$.
		HINT: You will need to calculate the magnitude of a vector to do this
		problem.
		\item  Use Matlab's cross command to find the angular momentum $L =
...
\section{Center of Mass Coordinates}
	...
	hence to $r_1 −r_2$). To evaluate these two dot products using Matlab's
...
\subsection*{\small{P8.6}}\label{P8.6}
	...
	call this kind of motion ''precession'' and it looks like \hyperref[Figure 8.2]{Fig. 8.2}.\\
	In general relativity the gravitational force law is not precisely inversesquare, so this kind of precession is expected to occur. Mercury's orbit
	has a small precession of this kind (the famous ''precession of the equinox
	of Mercury'' ) which has been measured for centuries. When Einstein's
...
\marginpar{
\includegraphics[scale = 0.6]{ch8zd1}
\captionof{figure}[] {Precession of the orbit,
non-inverse$-$square.}\label {Figure 8.2}
}
\end{document}