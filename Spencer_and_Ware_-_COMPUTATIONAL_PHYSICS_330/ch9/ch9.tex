...
\end{equation}
If you remember Euler's relation e
...
part of $g (\omega )$ is the overlap with sin$(\omega t)$\textsuperscript{1} Often, we aren't interested in the phase
...
\subsection*{P9.2}\label{P9.2}
 On the class web site is a file called ''Beethoven.wav'' that has the first four
notes of Beethoven's 5th symphony. Save it to your computer and listen to
...
\marginpar{
\includegraphics[width=\marginparwidth]{ch999}
\captionof{figure}[] {The power spectrum of
the first four notes of Beethoven's
5th symphony.}\label {Figure 9.1}
}
...
note is an ''E$-$flat,'' again played in octaves by the various stringed instruments (312 Hz, 156 Hz, and 78 Hz). Identify each of these peaks on the
...
\marginpar{
\includegraphics[width=\marginparwidth]{ch992}
\captionof{figure}[] {A string's fundamental mode of vibration has nodes
...
third harmonic, etc.}\label {Figure 9.2}
}
...
\end{equation}
which has a ''bump'' centered at $t_0$ with a width controlled by W . Because the
...
their sound becomes '' '', which simply means that the note isn't a very pure
...
can still enjoy ''Flight of the Bumblebee'' even when played by a tuba\textsuperscript{5}.\\
...
f = f .* win;
\end{lstlisting}
he transpose operator (') at the end of the first line switches the window
...
We've made the peaks so broad that they've smooshed into each other due
...
\marginpar{
\includegraphics[width=\marginparwidth]{ch93}
\captionof{figure}[] {Enter your data here}\label {Table 9.1 }
}

\footnotetext{\textsuperscript{3}This is not a mathematically rigorous uncertainty relation, but it illustrates the idea.}
\footnotetext{\textsuperscript{4}The length of the tuba also contributes to the ''muddyness'' of the sound, since it takes a while
for sound to propagate back and forth between the mouthpiece and the bell and set up the standing
wave. This causes a messy ''attack'' transient at the beginning of each note, which means you have
