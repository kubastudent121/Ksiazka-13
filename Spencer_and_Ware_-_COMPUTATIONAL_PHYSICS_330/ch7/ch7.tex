...
oscillating system is a simple pendulum. In this lab we'll study how the pendulum
...
simple harmonic oscillator. But it doesn't take a very large amplitude before the
...
you down.
\subsection*{P7.2}\label{P7.2}
 Use \hyperref[7.1]{Eq. 7.1} with Matlab to make a phase space diagram with Matlab's
...
\marginpar{
\includegraphics[scale = 0.6]{ch7zd}
\captionof{figure}[] {A phase space flow plot
for a pendulum.}\label {Figure 7.1}
}
...
\subsection*{P7.4}\label{P7.4}
	...
	goes to zero at $\theta _0 = \pi$. You'll need to use the online help to see the syntax
	for evaluating the elliptic integral function.

	Now let's solve the pendulum equation numerically using Matlab.

\subsection*{P7.5}\label{P7.5}
	Use Matlab's numerical differential equation solver ode45 to solve the pendulum, again with $\omega _0 = 1$ and initial conditions $\theta (0) = \theta _0$ and $\omega (0) = 0$. Plot
	...
	is not sinusoidal.
\subsection*{P7.6}\label{P7.6}
	Now let's study what happens when we add driving and damping.
\begin{enumerate}[label=\alph*]
	\item First we'll review what happens when we drive an undamped harmonic oscillator.Write a Matlab script that solves the driven oscillator equation
...
$\omega _\tau = 1$, and $\alpha = 0.1$. Start at rest and run for a long enough time that
	you can see that the pendulum amplitude doesn't simply go to infinity
...
\end{equation}\\
	...
	$0.90\omega _0 \rightarrow 1.05\omega _0$ and explain why $\omega = \omega _0$ doesn't give the largest
amplitude.
\end{enumerate}
\marginpar{
\includegraphics[scale = 0.6]{ch7zd55}
\captionof{figure}[] {\ref{Figure 7.4} Oscillation frequency
as a function of the maximum
amplitude $\theta _0$.}\label {Figure 7.4}
}
...
\section{Differential Equations in Mathematica}
	...
	you should be aware of. Let's take a break from Matlab and learn some of the
	basics in Mathematica.
\subsection*{P7.7}\label{P7.7}
	Read the section titled ''Symbolic solutions to ordinary differential equations'' in the Mathematica tutorial Differential equations with Mathematica
	(available on the Physics 330 course web page).
\subsection*{P7.8}\label{P7.8}
	...
	\item  Legendre's Equation
		...
		\end{equation}
	\end{enumerate}
\subsection*{P7.9}\label{P7.9}
	 Read the section titled ''Numerical solutions to ordinary differential equations'' in the Mathematica tutorial Differential equations with Mathematica
	\begin{enumerate}[label=\alph*]
	    ...
	    with $y(0) = 0$ and $v(0) \equiv y' (0)+0.1 .$ 
