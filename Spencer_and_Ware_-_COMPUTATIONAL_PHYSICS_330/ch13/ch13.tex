...
\chapter{Coupled Nonlinear Oscillators}
	...
	between the ''together'' and ''opposite'' motions is the cause of the two slightly
...
\subsection*{P13.2}\label{P13.2}
\marginpar{
\includegraphics[width=\marginparwidth]{ch131313133}
\captionof{figure}[] {Energy passes back
and forth between $\theta _1$ and $\theta _2$ due
to beating.}\label {Figure 13.1}
}
...
\item Now drive pendulum number 1 by applying a small negative torque
	...
	As in part (b), don't damp pendulum number 1; just keep the damping
...
\section{Coupled Wall Clocks}
	...
	on a wall, they tend to synchronize with each other. Let's see if we can make our
	equations of motion do this.
\subsection*{P13.3}\label{P13.3}
\begin{enumerate}[label=\alph*]
...
\item  According to the nonlinear dynamics literature, entrainment is an
	...
	\item Finally, let's make these clocks a little more realistic by (i) replacing
	...
	meaning that the clocks don't have to have exactly the same period to
...
\section{Solving Nonlinear Equations}
	...
	\begin{enumerate}[label=\alph*]
			...
			using Matlab's fzero command. You will have to give fzero a search
			...
			\item Solve the following set of equations using Matlab's fsolve command 
			$$x^{2}+y^{2}+z^{2}=139$$
			$$\dfrac{x}{x+y-z}=3$$
			$$x\sqrt{z}=(10-y)^{2}$$
	\end{enumerate}
