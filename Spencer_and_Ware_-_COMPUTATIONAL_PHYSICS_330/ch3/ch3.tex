...
\item Write a script that makes a Matlab surface plot of the ''mountain''
function \hyperref[Figure 3.1]{Fig. 3.1}:
\begin{equation}\label{3.1}
f(x,y)=e^{-\text{|}x-siny\text{|}}(1+\dfrac{1}{5}cos(\dfrac{x}{2})(1+\dfrac{4}{3+10y^{2}})
\end{equation}
\marginpar{
\includegraphics[width=\marginparwidth]{latexch3}
\captionof{figure}[] {The ''mountain'' function.}\label {Figure 3.1}

\includegraphics[width=\marginparwidth]{ch33}
\captionof{figure}[] { Streamlines and wind
velocity for the a wind velocity
field.
}\label {Figure 3.2}
}
...
your plot is solid black, don't use such a fine grid in x and y.
...
you what the ''flow'' velocity is at each point in space. At any point in time t, the
coordinate $[x(t), v(t)]$ gives the phase-space point that represents the ''state'' of
...
Part of the power of phase space flow plots is that you don't have to solve the
...
To this point, we've mostly relied on Matlab's built$-$in functions tied together with
some code to perform our work. As our numerical techniques advance, we'll need
...
This limit is called Euler's constant, often represented by the Greek
letter $\gamma$. To 15 digits, Euler's constant is
