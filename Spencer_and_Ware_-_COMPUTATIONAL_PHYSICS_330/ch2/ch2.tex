...
\end{lstlisting}
	...
	and then performs a ''bubble sort'' to order the array elements in A from
	...
	''bubble'' to the top of the list. Step through your code using the debugging
...
\section{Line Plots}
	...
	Let's look at some of the 1$-$dimensional plotting tools.
	
\subsection*{P2.2}\label{P2.2}
...
\item Write a script with a for loop that calculates the first 20 terms of the
	recursion relation
	$$a_1=1 ~;~ a_{n+1}=\left( \dfrac{n}{(n-\dfrac{1}{2)}(n+\dfrac{1}{2})} \right) a_n.$$
	And stores each value in an array a. Use Matlab's debugging commands to step through your code while you watch the values change
...
\section{How does a differential equation make a curve?}
	...
	of whether the solutions that a computer spits out are reasonable. If you don't
	develop good intuitive skills, the many differential equations you'll encounter in
...
\end{equation}
	...
	bigger y gets the bigger its slope gets. Let's consider the two possible cases for
	initial conditions.
...
\marginpar{
The bubble sort is not an efficient way to sort. Matlab's
...
This idea is referred to as ''explosive growth.'' Keeping in mind that with $p = 1$
...
equations too. Let's translate the simple harmonic oscillator equation
...
So what does \hyperref[2.4]{Eq. 2.4} say if $y = 1$ and $y' = 0$ to start? The first derivative is
...
start to smile and curve upward. It doesn't curve much at first because y is pretty
...
\end{enumerate}
\subsection*{P2.4}\label{P2.4}
Don't start this problem until after making all of your sketches in \hyperref[P2.3]{P2.3}.
\begin{enumerate}[label=\alph*]
...
\marginpar{
\includegraphics[width=\marginparwidth]{ch2zd}
\captionof{figure}[]{The explosive growth
function defined by \hyperref[2.3]{Eq. 2.3} for two
values of P.}\label {Figure 2.1}
}
